...
public class OperandFetch {
	...
	public void performOF() {
		if (IF_OF_Latch.isOF_enable()) {
			...
			// DES: If EX is busy with a multi-cycle operation, we don't know yet which
			// register it will write — conservatively stall
			if (EX_MA_Latch.isEX_busy()) {
				hazard = true;
			}

			// DES: If MA is busy waiting for memory (load), we don't know the result yet
			if (!hazard && MA_RW_Latch.isMA_busy()) {
				hazard = true;
			}

			// Check against instruction in EX_MA latch (just finished EX)
			if (!hazard && EX_MA_Latch.isMA_enable()) {
			...
			IF_OF_Latch.setOF_enable(false);/*
											 * I added IF_OF_Latch.setOF_enable(false); to "consume" the instruction
											 * once it has been successfully decoded.
											 * 
											 * In a pipeline, once the OperandFetch stage finishes its work and passes
											 * the decoded data to the Execute
											 * stage (
											 * setEX_enable(true)
											 * ), it must mark its own input latch as empty.
											 * 
											 * Why it's necessary: If the pipeline stalls (for example, if
											 * InstructionFetch
											 * is disabled due to a data hazard), the
											 * OF
											 * enable bit would stay true. Without this line, the
											 * OperandFetch
											 * stage would re-decode the same instruction in the next cycle and send a
											 * duplicate to the
											 * Execute
											 * stage, leading to incorrect results.
											 * 
											 * By setting it to false, we ensure each instruction is processed by the
											 * OF
											 * stage exactly once. It will only become true again when the
											 * InstructionFetch
											 * stage successfully fetches a new instruction.
											 */
